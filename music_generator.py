#!/usr/bin/env python3
"""IA MUSIC GENERATOR PRO - Salva em song_output/ numeradas"""
import os, sys, json
import numpy as np
...
MODEL_SIZES = {
    "nano": {"params": "~50 mil"},
    "small": {"params": "~500 mil"},
    "medium": {"params": "~5 milhões"},
    "large": {"params": "~20 milhões"},
    "xlarge": {"params": "~50 milhões"},
}
...
def get_next_song_number():
    ...
    for f in existing:
        try:
            numbers.append(int(f.replace('.wav', '')))
        except ValueError:
            pass
...
def violin_note(freq, duration, sr=44100):
    ...
    if 0 < atk < len(t):
        envelope[:atk] = np.linspace(0, 1, atk) ** 0.5
...
def bitcrush(audio, bits=8, rate_div=4):
    ...
    reduced = audio[::rate_div]
    upsampled = np.repeat(reduced, rate_div)
    ...
    levels = 2 ** bits
    return np.round(upsampled * levels) / levels
...
def reese_bass(freq, duration, sr=44100, detune=0.03):
    t = np.linspace(0, duration, int(duration * sr), endpoint=False)
    saw1 = 2 * (t * freq * (1-detune) % 1) - 1
    saw2 = 2 * (t * freq * (1+detune) % 1) - 1
...
def sub_808(freq, duration, sr=44100):
    ...
    phase = 2 * np.pi * np.cumsum(freq_curve) / sr
    signal = np.tanh(np.sin(phase) * 1.5)
    return signal * np.exp(-t * 4)
...
def amen_break(sr=44100, tempo_factor=1.0):
    ...
    kick_s = kick_drum(sr)[:int(0.12*sr)]
    snare_s = snare_drum(sr)[:int(0.1*sr)]
    ghost_s = snare_drum(sr)[:int(0.06*sr)] * 0.4
    hat_s = hihat(sr)[:int(0.04*sr)]
    for i, (hit, vel) in enumerate(pattern):
        ...
        vel *= np.random.uniform(0.85, 1.1)
        end = min(pos + len(sample), total_samples)
        if pos < total_samples:
            output[pos:end] += sample[:end-pos] * vel
...
CHORD_PROGRESSIONS = {
    'epic': [[0,2,4],[5,0,2],[3,5,0],[4,6,1]],
    'pop': [[0,2,4],[4,6,1],[5,0,2],[3,5,0]],
    'sad': [[5,0,2],[3,5,0],[0,2,4],[4,6,1]],
}
...
KEYWORDS = {
    ...
    "épico": {"intensity": 0.9, "bpm_mult": 1.2, "orchestral": 2.0},
    "epico": {"intensity": 0.9, "bpm_mult": 1.2, "orchestral": 2.0},
    "sombrio": {"intensity": 0.7, "minor": True},
    "dark": {"intensity": 0.7, "minor": True},
    "feliz": {"major": True, "bpm_mult": 1.1},
    "triste": {"minor": True, "bpm_mult": 0.7},
    "guitarra": {"guitar": 2.0, "distortion": 1.5},
    "violão": {"guitar": 2.0}, "violao": {"guitar": 2.0},
    "piano": {"piano": 2.5},
    "bateria": {"drums": 2.5}, "tambores": {"drums": 2.5},
    "violino": {"strings": 2.5, "orchestral": 1.8},
    "synth": {"synth": 2.0}, "sintetizador": {"synth": 2.0},
    "baixo": {"bass": 2.5}, "flauta": {"flute": 2.0},
    "rock": {"guitar": 2.0, "drums": 2.0, "distortion": 1.5},
    "metal": {"guitar": 2.5, "drums": 2.5, "distortion": 2.0},
    "jazz": {"jazz": 2.5, "piano": 2.0},
    "eletrônica": {"synth": 2.5}, "eletronica": {"synth": 2.5},
    "clássica": {"orchestral": 2.5, "strings": 2.0, "piano": 2.0},
    "classica": {"orchestral": 2.5, "strings": 2.0, "piano": 2.0},
    ...
    "rápido": {"bpm_mult": 1.4}, "rapido": {"bpm_mult": 1.4},
    "lento": {"bpm_mult": 0.6}, "devagar": {"bpm_mult": 0.6},
    "suave": {"intensity": 0.5}, "pesado": {"intensity": 1.1, "drums": 1.5},
    "breakcore": {"breakcore": 3.0, "intensity": 1.0, "bpm_mult": 1.8},
    "amen": {"breakcore": 2.5}, "glitch": {"breakcore": 2.0},
    "jungle": {"breakcore": 2.0, "bpm_mult": 1.5},
...
def interpret_prompt(prompt):
    ...
    params = {"intensity": 0.7, "bpm_mult": 1.0, "orchestral": 1.0, "guitar": 1.0,
              "piano": 1.0, "drums": 1.0, "strings": 1.0, "synth": 1.0,
              "distortion": 1.0, "bass": 1.0, "flute": 1.0, "brass": 1.0,
              "jazz": 1.0, "minor": False, "major": True, "breakcore": 0.0}
    matched = []
...
def generate_breakcore(duration, sr=44100, intensity=1.0):
    ...
    while pos < total_samples:
        ...
        if np.random.random() < 0.4:
            chunk = np.tanh(chunk * np.random.uniform(2, 6))
        ...
        if np.random.random() < 0.24:
            pos += np.random.randint(int(0.01*sr), int(0.08*sr))
    ...
    for i in range(int(duration / beat_dur)):
        ...
        if np.random.random() < 0.6:
            ...
            if np.random.random() < 0.5:
                bass_note = reese_bass(freq, note_dur, sr)
            else:
                bass_note = sub_808(freq, note_dur, sr)
            end = min(pos + len(bass_note), total_samples)
            if pos < total_samples:
                bass_track[pos:end] += bass_note[:end-pos] * 0.7
    ...
    mix = np.tanh(mix * 2.0)
    mix = soft_compress(mix, 0.35, 5.0)
...
def generate_realistic_music(duration, params, base_bpm=120, sr=44100):
    total_samples = int(duration * sr)
    bpm = base_bpm * params.get('bpm_mult', 1.0)
    beat_duration = 60.0 / bpm
    print(f"Gerando: {duration}s, BPM={bpm:.0f}")
    if params.get('minor', False):
        scale, base_freq, progression = SCALES['minor'], 220.0, CHORD_PROGRESSIONS['sad']
    else:
...
def batch_generate():
    ...
    model_info = MODEL_SIZES.get(args.model_size, MODEL_SIZES["xlarge"])
    print("=" * 60)
    print("IA MUSIC GENERATOR PRO - MODO BATCH")
    print(f"   Modelo: {args.model_size} ({model_info['params']} params)")
    print("=" * 60)
    if args.prompt:
        ...
        metadata = {"prompt": args.prompt, "duration": args.duration, "model_size": args.model_size}
    else:
        style_map = {
        ...
        metadata = {"style": args.style, "duration": args.duration, "model_size": args.model_size}
...
def menu():
    clear_screen()
    print("=" * 50)
    print("IA MUSIC GENERATOR PRO")
    print(f"Musicas em: {OUTPUT_DIR}/")
    print("=" * 50)
    os.makedirs(OUTPUT_DIR, exist_ok=True)
